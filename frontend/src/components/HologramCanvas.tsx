  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 80; i++) {
      particles.push({
        ...
        hue: Math.random() > 0.5 ? 200 : 330 // blue or magenta
      });
    }
    ...
    const draw = () => {
      ...
      bgGrad.addColorStop(0, 'rgba(0, 153, 255, 0.06)');
      bgGrad.addColorStop(0.4, 'rgba(255, 0, 128, 0.03)');
      ...
      outerGlow.addColorStop(0, `rgba(0, 153, 255, ${0.3 * pulse})`);
      outerGlow.addColorStop(0.5, `rgba(255, 0, 128, ${0.1 * pulse})`);
      ...
      coreGrad.addColorStop(0.3, `rgba(0, 200, 255, ${0.7 * pulse})`);
      coreGrad.addColorStop(0.7, `rgba(255, 0, 128, ${0.3 * pulse})`);
      ...
      // ── Rotating ring arcs ──
      const drawArc = (radius: number, speed: number, color: string, lw: number, startAngle: number, sweep: number) => {
        ctx.save();
        ctx.translate(cx, cy);
      ...
      // Multiple partial arcs at various radii
      drawArc(50, 0.6, `rgba(0, 153, 255, ${0.5 * pulse})`, 1.5, 0, Math.PI * 0.7);
      drawArc(50, 0.6, `rgba(0, 153, 255, ${0.3 * pulse})`, 1.5, Math.PI, Math.PI * 0.5);
      drawArc(65, -0.4, `rgba(255, 0, 128, ${0.4 * pulse})`, 1, 0.5, Math.PI * 0.8);
      drawArc(65, -0.4, `rgba(255, 0, 128, ${0.25 * pulse})`, 1, Math.PI + 0.5, Math.PI * 0.6);
      drawArc(82, 0.25, `rgba(0, 153, 255, ${0.2 * pulse})`, 0.8, 0, Math.PI * 1.2);
      drawArc(97, -0.15, `rgba(255, 0, 128, ${0.15 * pulse})`, 0.5, 1, Math.PI * 0.9);

      // ── Data tick marks on outer ring ──
      ctx.save();
      ctx.translate(cx, cy);
      ctx.rotate(time * 0.1);
      for (let i = 0; i < 36; i++) {
        ...
        ctx.strokeStyle = `rgba(0, 153, 255, ${i % 3 === 0 ? 0.3 : 0.1})`;
      ...
      drawNode(50, 0.6, 0, 200, 255, 0.9, 2.5);
      drawNode(65, -0.4, 255, 0, 128, 0.8, 2);
      drawNode(82, 0.25, 0, 153, 255, 0.6, 1.8);
      ...
      particles.forEach(p => {
        ...
        const col = p.hue === 200 ? '0, 153, 255' : '255, 0, 128';
